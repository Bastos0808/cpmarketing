...
class WebGLHandler {
    ...
    constructor(public cn: HTMLCanvasElement, fragmentShaderSource: string) {
        this.gl = cn.getContext('webgl2')!;
    ...
    resize = () => {
        ...
        this.cn.height = this.cn.parentElement?.clientHeight || window.innerHeight;
...
const WebGLBackground = ({ children }: { children: React.ReactNode }) => {
    const canvasRef = useRef<HTMLCanvasElement>(null);

    useEffect(() => {
        if (canvasRef.current) {
            const webGL = new WebGLHandler(canvasRef.current, fragmentShaderSource);
